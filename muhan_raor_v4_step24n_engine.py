"""
Step24N - 라오어 무한매수법 4.0 원문기반 매도예약 보강 엔진
...
def normalize_ticker(ticker: str) -> str:
    ...
    if ticker not in SUPPORTED_TICKERS:
        raise ValueError(f"Step24N 원문엔진은 현재 TQQQ/SOXL만 지원합니다. 입력 티커={ticker}")
    return ticker
...
def normalize_split(split_count: int) -> int:
    ...
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(
            "Step24N 원문엔진은 현재 별% 공식이 명시된 20/40분할만 지원합니다. "
            f"입력 분할={split_count}"
        )
    return split_count
...
def run_raor_infinite4_step24n_core(
    ...
    # Step24N에서는 백테스트 비교를 위해 UI에서 지정가 전량매도 수익률을 선택할 수 있게 하되,
    # 기본값은 원문값을 유지한다.
    designated_sell_pct = (
        ...
        if state.shares > EPS:
            ...
            if designated_hit:
                ...
                execute_sell(
                    ...
                    sell_fraction=0.75,
                    fee_rate=fee_rate,
                    reason=f"Step24N 지정가매도(커스텀): 보유수량 75% {ticker} +{designated_sell_pct:.0f}% / 지정가 {designated_sell_price_before:.2f} / phase={phase_before}",
            ...
            if state.shares > EPS and star_loc_hit:
                ...
                execute_sell(
                    ...
                    fee_rate=fee_rate,
                    reason=f"Step24N {sell_label}: 별% LOC 매도점 {star_price_before:.2f} / phase={phase_before}",
                    trade_logs=trade_logs,
                )
                if state.shares > EPS:
...
run_raor_infinite4_step24n_backtest = run_raor_infinite4_step24n_core
