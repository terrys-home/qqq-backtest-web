"""
Step24B - 라오어 무한매수법 4.0 원문기반 확장 엔진
...
def normalize_ticker(ticker: str) -> str:
    ...
    if ticker not in SUPPORTED_TICKERS:
        raise ValueError(f"Step24B 원문엔진은 현재 TQQQ/SOXL만 지원합니다. 입력 티커={ticker}")
    return ticker
...
def normalize_split(split_count: int) -> int:
    ...
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(
            "Step24B 원문엔진은 현재 별% 공식이 명시된 20/40분할만 지원합니다. "
            f"입력 분할={split_count}"
        )
    return split_count
...
def run_raor_infinite4_step24b_core(
        ...
        if state.shares > EPS:
            if enable_quarter_sell and star_price_before > 0 and close_price >= star_price_before:
                ...
                execute_sell(
                    ...
                    sell_fraction=0.25,
                    fee_rate=fee_rate,
                    reason=f"Step24B 쿼터매도 LOC 별%매도점 {star_price_before:.2f} / phase={phase_before}",
            ...
            # 전반전에서도 사용자가 켜면 계산은 가능하지만, Step24B의 핵심 분기는 후반전/소진모드에서만 실행한다.
            ...
            if (
                ...
                execute_sell(
                    ...
                    sell_fraction=1.0,
                    fee_rate=fee_rate,
                    reason=f"Step24B 후반전/소진모드 지정가매도 {ticker} +{designated_sell_pct:.0f}%",
                    trade_logs=trade_logs,
                )
...
run_raor_infinite4_step24b_backtest = run_raor_infinite4_step24b_core
