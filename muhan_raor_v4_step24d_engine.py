"""
Step24D - 라오어 무한매수법 4.0 원문기반 확장 엔진
...
8) 쿼터매도: T>1이면 보유수량 25% 매도, T = 기존 T * 0.75 / 잔여 T<=1이면 전량매도 후 주기 완료
...
- 쿼터매도는 전반전/후반전/소진모드 모두 별% 매도점 LOC 체결 조건으로 처리하되, 잔여 T<=1이면 전량매도로 주기 완료
- 후반전/소진모드에서는 지정가매도와 LOC 쿼터매도 체결 후보를 모두 계산하고, 과매도 방지를 위해 LOC 쿼터매도 후 지정가 전량/잔량 매도 순서로 처리
...
def normalize_ticker(ticker: str) -> str:
    ...
    if ticker not in SUPPORTED_TICKERS:
        raise ValueError(f"Step24D 원문엔진은 현재 TQQQ/SOXL만 지원합니다. 입력 티커={ticker}")
    return ticker
...
def normalize_split(split_count: int) -> int:
    ...
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(
            "Step24D 원문엔진은 현재 별% 공식이 명시된 20/40분할만 지원합니다. "
            f"입력 분할={split_count}"
        )
    return split_count
...
def run_raor_infinite4_step24d_core(
    ...
    for i, row in df.iterrows():
        ...
        # 1) 매도: 쿼터 LOC와 잔여 1T 이하 전량청산을 분리 계산
        # 중요 수정(Step24D hotfix):
        # - 기존 코드는 별% 매도점 도달 시 항상 25%만 팔아서 T가 1 이하로 작아져도
        #   0으로 닫히지 않는 구조였습니다. 그래서 완료 주기가 비정상적으로 적게 보였습니다.
        # - 쿼터매도는 T>1일 때의 부분청산으로 보고, 잔여 T가 1 이하인 상태에서
        #   매도점에 도달하면 남은 수량을 전량 청산해 해당 무한매수 주기를 완료 처리합니다.
        if state.shares > EPS:
            if enable_quarter_sell and star_price_before > 0 and close_price >= star_price_before:
                old_T = state.T
                if old_T <= 1.0 + EPS:
                    sell_fraction = 1.0
                    sell_label = "잔여1T전량매도LOC"
                    sell_reason = f"Step24D 잔여T<=1 전량매도 LOC 별%매도점 {star_price_before:.2f} / phase={phase_before}"
                else:
                    sell_fraction = 0.25
                    sell_label = "쿼터매도LOC"
                    sell_reason = f"Step24D 쿼터매도 LOC 별%매도점 {star_price_before:.2f} / phase={phase_before}"
            ...
            # 후반전/소진모드 지정가 분기: 원문 목표가 TQQQ 15%, SOXL 20% 기준.
            # 전반전에서도 사용자가 켜면 계산은 가능하지만, Step24D의 핵심 분기는 후반전/소진모드에서만 실행한다.
            phase_after_quarter = current_phase(state.T, split_count, state.cash)
            avg_after_quarter = state.avg_price
            designated_sell_price_after = avg_after_quarter * (1.0 + designated_sell_pct / 100.0) if avg_after_quarter > 0 else 0.0
            if (
                state.shares > EPS
                and enable_designated_sell
                and phase_after_quarter in {"후반전", "소진모드"}
                and designated_sell_price_after > 0
                and high_price >= designated_sell_price_after
            ):
                old_T = state.T
                execute_sell(
                    state=state,
                    date=date,
                    sell_price=designated_sell_price_after,
                    sell_fraction=1.0,
                    fee_rate=fee_rate,
                    reason=f"Step24D 후반전/소진모드 지정가매도 {ticker} +{designated_sell_pct:.0f}%",
                    trade_logs=trade_logs,
                )
                action_parts.append("후반전지정가매도")
                debug_parts.append(f"지정가매도: T {old_T:.4f}→{state.T:.4f}, 지정가={designated_sell_price_after:.2f}")
                sold_today = True
...
run_raor_infinite4_step24d_backtest = run_raor_infinite4_step24d_core
