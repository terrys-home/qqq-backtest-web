"""
Step24H - 라오어 무한매수법 4.0 원문기반 매도예약 보강 엔진
...
- 같은 날 지정가와 별LOC가 모두 충족되면 3/4 지정가 체결 후 남은 1/4 별LOC 체결로 주기 완료 처리
...
def normalize_ticker(ticker: str) -> str:
    ...
    if ticker not in SUPPORTED_TICKERS:
        raise ValueError(f"Step24H 원문엔진은 현재 TQQQ/SOXL만 지원합니다. 입력 티커={ticker}")
    return ticker
...
def normalize_split(split_count: int) -> int:
    ...
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(
            "Step24H 원문엔진은 현재 별% 공식이 명시된 20/40분할만 지원합니다. "
            f"입력 분할={split_count}"
        )
    return split_count
...
def run_raor_infinite4_step24h_core(
    ...
    # Step24H에서는 백테스트 비교를 위해 UI에서 지정가 전량매도 수익률을 선택할 수 있게 하되,
    # 기본값은 원문값을 유지한다.
    designated_sell_pct = (
    ...
    for i, row in df.iterrows():
        ...
        if state.shares > EPS:
            ...
            if star_price_before > 0 and designated_sell_price_before > 0 and star_price_before > designated_sell_price_before + 1e-8:
                debug_parts.append(f"경고: 별LOC({star_price_before:.2f}) > 지정가({designated_sell_price_before:.2f}) - 별%/T 계산 재확인 필요")
            ...
            if designated_hit:
                ...
                execute_sell(
                    ...
                    sell_fraction=0.75,
                    fee_rate=fee_rate,
                    reason=f"Step24H 지정가매도(커스텀): 보유수량 75% {ticker} +{designated_sell_pct:.0f}% / 지정가 {designated_sell_price_before:.2f} / phase={phase_before}",
            ...
            if state.shares > EPS and star_loc_hit:
                ...
                execute_sell(
                    ...
                    fee_rate=fee_rate,
                    reason=f"Step24H {sell_label}: 별% LOC 매도점 {star_price_before:.2f} / phase={phase_before}",
                    trade_logs=trade_logs,
                )
                if state.shares > EPS:
        ...
        same_day_multi_signal = signal_count >= 2 or executed_count >= 2
        if same_day_multi_signal:
            debug_parts.append(
                f"SameDayMultiSignal: 지정가신호={'Y' if designated_hit else 'N'}, "
                f"별LOC신호={'Y' if star_loc_hit else 'N'}, 매수LOC신호={'Y' if buy_loc_signal else 'N'}, "
                "일봉 데이터는 장중 선후관계를 알 수 없으므로 보수적 검증 필요"
            )
        ...
        results.append({
            ...
            "SameDayMultiSignal": bool(same_day_multi_signal),
            "ExecutionPriorityNote": "매도예약 우선 기록, 같은 날 매수는 기본 차단" if same_day_multi_signal else "",
...
run_raor_infinite4_step24h_backtest = run_raor_infinite4_step24h_core
